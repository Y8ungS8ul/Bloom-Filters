 ...
 * Автоматически подбирает оптимальные параметры (размер битового массива и количество
 * хеш-функций) на основе заданного количества элементов и допустимой вероятности
 * ложноположительных срабатываний.
 * 
 * @tparam T - тип хранимых элементов (обычно std::string)
 * 
 * Отличительные особенности:
 * - Автоматический расчёт размера и количества хеш-функций по формуле
...
private:
    ...
    std::unordered_map<size_t, size_t> collisionMap;           // Для подсчёта коллизий (сколько раз бит был установлен в 1)
...
public:
     ...
     * Автоматически вычисляет оптимальные параметры фильтра на основе:
     * - ожидаемого количества элементов (n)
     * - допустимой вероятности ложноположительного срабатывания (P)
     * 
     * Формулы:
     ...
     * 
     * @throws std::bad_alloc - при невозможности выделить память под битовый массив
     */
    DynBloomFilter(int numElements, double falsePositiveRate)
    {
        ...
        // Используем комбинацию различных хеш-алгоритмов для лучшего распределения
        for (int i = 0; i < k; ++i) {
            if (i == 1)
            {
                // Вторая хеш-функция - простая кольцевая
                addHashFunction([this](const T& item) { return this->hashFunction1(item); });
            }
            else if (i == 2)
            {
                // Третья хеш-функция - другая кольцевая
                addHashFunction([this](const T& item) { return this->hashFunction2(item); });
            }
            else if (i == 3)
            {
                // Четвёртая хеш-функция - MurmurHash (качественная не криптографическая)
     ...
     * Используется для обеспечения высокой криптостойкости и равномерного распределения
     * 
     * @param str - входная строка
     * @return size_t - хеш-значение (свёрнутое из 256-битного хеша)
     */
    size_t hashSHA256(const std::string& str) {
        SHA256 sha256;
        std::string hashStr = sha256(str);
        
        // Преобразование строки хеша в size_t (свёртка)
        size_t hash = 0;
        for (char c : hashStr) {
     ...
     * 
     * @param str - входная строка
     * @return size_t - хеш-значение (свёрнутое из 128-битного хеша)
     */
     ...
     * Отличается высокой скоростью работы и хорошим распределением
     * 
     * @param myString - входная строка
     * @param len - длина строки
     * @param seed - начальное значение (константа)
     ...
     * Возвращает size_t для совместимости с другими хеш-функциями
     * 
     * @param str - входная строка
     * @return size_t - хеш-значение
     */
    ...
    void insert(const T& item)
    {
        ...
        for (const auto& hashFunction : hashFunctions)
        {
            ...
            collisionMap[index]++; // Увеличиваем счётчик коллизий для этого индекса
            indices.push_back(index); // Сохраняем индекс для дальнейшего анализа
        }
    ...
    /**
     * Подсчёт коллизий (относительных)
